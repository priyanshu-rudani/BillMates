...
def pay_salery_ui(parent = None):
    ASSETS_PATH = Path(generate_path(root_path, "UI", "assets", "frame2"))
    ...
    def add_payment():
        ...
        Cus_name = staff_name.get().strip()  # Remove leading/trailing spaces
        entity_type = 'Salaries'
        ...
        if not Cus_name:
            messagebox.showerror("Error", "Staff name is required!", parent=window)
            return
        ...
        if entity_id:
            ...
            Added_Balance = float(total_amount)
            ...
            run_query("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (entity_id, entity_type, date, total_amount, payment_mode, reference_no, 'Salary'))

            # Show success message
            messagebox.showinfo("Success", "Salaries Payment added successfully!", parent=window)

        else:
            # Show error message if client not found
            messagebox.showerror("Error", "No Staff found with the given name and type.", parent=window)
    ...
    window.title("Add Salary Payment")
    ...
    staff_name = ttk.Combobox(
        window, 
        values=fetch_client_names('Staff'), 
        state="readonly", 
