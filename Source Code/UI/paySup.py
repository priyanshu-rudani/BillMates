...
def supplier_payments_out(parent = None):
    ASSETS_PATH = Path(generate_path(root_path, "UI", "assets", "frame3"))
    ...
    def add_payment():
        ...
        total_amount = entry_4.get().strip()  # Ensure no leading/trailing whitespace
        ...
        if not Cus_name:
            messagebox.showerror("Error", "Customer name is required!", parent=window)
            return
        ...
        if entity_id:
            ...
            invoices = fetch_data(f"""SELECT invoice_id, total, paid, remaining FROM Purchase WHERE client_id = {entity_id} AND remaining > 0 AND paid_status != 'Paid' ORDER BY created_at ASC""")
            ...
            for invoice in invoices:
                ...
                if amount >= remaining:
                    ...
                    run_query("UPDATE Purchase SET paid = ?, remaining = 0, paid_status = ? WHERE invoice_id = ?", (total, 'Paid', invoice_id))
                    amount -= remaining
                else:
                    # Partial payment
                    run_query("UPDATE Purchase SET paid = ?, remaining = ?, paid_status = ? WHERE invoice_id = ?", (paid + amount, remaining - amount, 'Partially Paid', invoice_id))
                    amount = 0
            ...
            query = """
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """
            params = (entity_id, "Expense", date, total_amount, payment_mode, reference_no, "Payment For Purchase")
            ...
            messagebox.showerror("Error", "No Supplier found with the given name and type.", parent=window)
    ...
    window.title("Add Supplier Payments")
    ...
    customer_name = ttk.Combobox(
        window, 
        values=fetch_client_names('Supplier'), 
        state="readonly", 
